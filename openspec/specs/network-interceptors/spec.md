...
Defines the interceptor chain of `vm_network`: injectable auth-token attachment,
default-header merging, retry with backoff, flag-controlled logging with secret
redaction, and consumer-supplied custom interceptors expressed via a `vm_network`
abstraction (not a Dio type), including their ordering guarantees.
