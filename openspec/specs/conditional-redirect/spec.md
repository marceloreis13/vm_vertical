...
Defines how `vm_navigation` resolves a failing route guard: through `go_router`'s
`redirect` mechanism, sending navigation to a configured fallback route instead of
rendering the protected screen. Scopes "deep link" behavior in this module to this
redirect only — no external URI/deep-link parsing.
