...
Defines the fixed design tokens (spacing, radius, elevation, motion, typography) that
`vm_storyboard` exposes as a single `ThemeExtension`, identical across every app on the
platform.
