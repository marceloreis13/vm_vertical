...
Defines how `vm_storyboard` derives light/dark `ThemeData` from its fixed tokens and an
app-injected `VmThemeConfig`, and the exact subset an app may customize (color palette, logo,
optional font family) versus what stays fixed.
