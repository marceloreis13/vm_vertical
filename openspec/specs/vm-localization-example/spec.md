# vm-localization-example

## Purpose
`vm_localization` ships a standalone, self-contained `example/` app — following the
same pattern as `vm_storage`/`vm_network`/`vm_navigation` — demonstrating the module's
full public API through a language selector that switches date/number/currency formats
at runtime across `pt_BR` (default) and `en`.
