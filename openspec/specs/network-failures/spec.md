...
Defines the `Result`/`Failure` model used across `vm_network`: a `Result<S, F>` success/failure
wrapper, a sealed `Failure` taxonomy covering network, timeout, server, parsing, and
unauthorized cases, and the guarantee that no raw exception escapes the client.
