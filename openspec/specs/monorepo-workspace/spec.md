...
Defines the repository layout (`apps/`, `packages/`, `openspec/`, `docs/`) and the Pub Workspace setup that resolves every transitive dependency to a single version with one lockfile at the repository root, orchestrating bootstrap, analysis, tests and versioning across packages.
